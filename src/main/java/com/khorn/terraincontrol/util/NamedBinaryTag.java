...
public class NamedBinaryTag {
	...
	public Type getListType() { return listType; }
	...
	public void removeSubTag(NamedBinaryTag tag) {
		...
		for (int i = 0; i < subtags.length; i++) {
			if (subtags[i] == tag) {
				...
				if (subtags[i].type == Type.TAG_List || subtags[i].type == Type.TAG_Compound) { subtags[i].removeSubTag(tag); }
			}
		}
	}
	...
	public NamedBinaryTag getTag(String name) {
		if (type != Type.TAG_Compound) { return null; }
	...
	public NamedBinaryTag[] values() {
		if (type != Type.TAG_Compound && type != Type.TAG_List) { return new NamedBinaryTag[0]; }
