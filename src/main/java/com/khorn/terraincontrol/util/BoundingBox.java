 ...
 *
 * <p>
 * Bounding boxes are mutable. Usually bounding boxes gradually expand
...
public final class BoundingBox implements Cloneable {
	...
	/**
	 * Gets a new bounding box that is rotated 90 degrees to the original. The
	 * original bounding box will not be modified.
	 * 
	...
	public void expandToFit(int x, int y, int z) {
		...
		else if (x >= maxX) { maxX = x + 1; }
		if (y < minY) {
			minY = y;
		}
		else if (y >= maxY) { maxY = y + 1; }
		if (z < minZ) {
			minZ = z;
		}
		else if (z >= maxZ) { maxZ = z + 1; }
	}
	 ...
	 */
	public boolean isEmpty() { return minX == maxX || minY == maxY || minZ == maxZ; }
	 ...
	 * @return The depth.
	 */
	public int getDepth() { return maxZ - minZ; }
	...
	public String toString() {
		return "BoundingBox[minX=" + minX + ", minY=" + minY + ", minZ=" + minZ + ", width=" + getWidth() + ", height=" + getHeight()
				+ ", depth=" + getDepth() + "]";
	}
