...
public class EventManager extends EventHandler {
	 ...
	 * da52e52d1707e596cf8c7307b3ebc40a7682a5d3/MODSRC/vazkii/botania/common/core/
	 * handler/BiomeDecorationHandler.java#L24
	 * https://github.com/modmuss50/Transcraft/blob/
	 * 71b89d89b209619c514c3068b0268d8b1704b8ea/src/main/java/modmuss50/mods/
	 * transcraft/WorldGen/FlowerGen.java#L15
	 * https://github.com/Andy5823/Hardmode/blob/
	 * 029e8ca9673f935ed12c8c46e23ab50dc5a1281c/common/Hardmode/WorldGen/
	 * HardmodeWorldGenHandler.java#L11
	 *
	 * A simple search on Github should yield many more results.
	 */

	// Two maps, key is world name, value is boolean
	private final Map<String, Boolean> hasOreGenBegun = new HashMap<>();
	private final Map<String, Boolean> hasDecorationBegun = new HashMap<>();

	@Override
	public boolean onResourceProcess(Resource resource, LocalWorld localWorld, Random random, boolean villageInChunk, int chunkX,
			int chunkZ, boolean isCancelled) {
		...
		if (resource instanceof DungeonGen || resource instanceof SmallLakeGen || resource instanceof UndergroundLakeGen
				|| resource instanceof LiquidGen || resource instanceof CustomObjectGen) {
			...
			if (!hasOreGenerationBegun(world)) {
				...
				MinecraftForge.ORE_GEN_BUS.post(new OreGenEvent.Pre(world.getWorld(), random, blockX, blockZ));
			...
			if (!hasDecorationBegun(world)) {
				...
				MinecraftForge.EVENT_BUS.post(new DecorateBiomeEvent.Pre(world.getWorld(), random, blockX, chunkZ * CHUNK_Z_SIZE));
	...
	public void onPopulateStart(LocalWorld localWorld, Random random, boolean villageInChunk, int chunkX, int chunkZ) {
		...
		// Fire event
		PopulateChunkEvent forgeEvent = new PopulateChunkEvent.Pre(world.getChunkGenerator(), world.getWorld(), random,
				chunkX * CHUNK_X_SIZE, chunkZ * CHUNK_Z_SIZE, villageInChunk);
	...
	public void onPopulateEnd(LocalWorld localWorld, Random random, boolean villageInChunk, int chunkX, int chunkZ) {
		...
		// There is no need to call GameRegistry.generateWorld, because it is done by
		// the Chunk Provider in Forge.
	}
	...
	private Decorate.EventType getDecorateEventType(LocalMaterialData block) {
		...
		if (block.isMaterial(DefaultMaterial.RED_ROSE) || block.isMaterial(DefaultMaterial.YELLOW_FLOWER)
				|| block.isMaterial(DefaultMaterial.DOUBLE_PLANT))
