...
public class MooshroomSpawnHandler {
	
	public MooshroomSpawnHandler() { MinecraftForge.EVENT_BUS.register(this); }
	
	@SubscribeEvent
	public void checkSpawnEvent(LivingSpawnEvent.CheckSpawn event) {
		if (!(event.entity instanceof EntityMooshroom)) { return; }
		final EntityAnimal ent = (EntityAnimal) event.entity;
		final World world = event.world;
		final int x = (int) event.x, y = (int) event.y, z = (int) event.z;
