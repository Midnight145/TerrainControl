...
public class VeinGen extends Resource {
	...
	protected void spawnInChunk(LocalWorld world, Random random, boolean villageInChunk, ChunkCoordinate chunkCoord) {
		...
		for (int searchChunkX = currentChunkX - searchRadius; searchChunkX < currentChunkX + searchRadius; searchChunkX++) {
			for (int searchChunkZ = currentChunkZ - searchRadius; searchChunkZ < currentChunkZ + searchRadius; searchChunkZ++) {
				...
				if (vein != null && vein.reachesChunk(currentChunkX, currentChunkZ)) { vein.spawn(world, random, chunkCoord, this); }
			}
		}
	}
	...
	public Vein getVeinStartInChunk(LocalWorld world, int chunkX, int chunkZ) {
		...
		Random random = RandomHelper.getRandomForCoords(chunkX, chunkZ,
				material.hashCode() * (minRadius + maxRadius + 100) + world.getSeed());
	...
	public boolean equals(Object other) {
		...
		return this.veinRarity == compare.veinRarity && this.minRadius == compare.minRadius && this.maxRadius == compare.maxRadius
				&& this.oreSize == compare.oreSize && this.oreFrequency == compare.oreFrequency && this.oreRarity == compare.oreRarity
				&& this.minAltitude == compare.minAltitude && this.maxAltitude == compare.maxAltitude
				&& (this.sourceBlocks == null ? this.sourceBlocks == compare.sourceBlocks : this.sourceBlocks.equals(compare.sourceBlocks));
	}
