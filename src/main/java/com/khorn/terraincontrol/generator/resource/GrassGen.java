...
public class GrassGen extends Resource {
	...
	protected void spawnGrouped(LocalWorld world, Random random, ChunkCoordinate chunkCoord) {
		if (random.nextDouble() * 100.0 <= this.rarity) {
			...
			// Fix y position
			while (((id = world.getMaterial(centerX, centerY, centerZ)).isAir() || id.isMaterial(DefaultMaterial.LEAVES)
					|| id.isMaterial(DefaultMaterial.LEAVES_2)) && (centerY > 0)) {
				centerY--;
			}
			centerY++;
			...
			for (int i = 0; i < frequency * 4; i++) {
				...
				if (world.isEmpty(x, y, z) && this.sourceBlocks.contains(world.getMaterial(x, y - 1, z))) { plant.spawn(world, x, y, z); }
	...
	protected void spawnNotGrouped(LocalWorld world, Random random, ChunkCoordinate chunkCoord) {
		for (int t = 0; t < frequency; t++) {
			...
			while (((material = world.getMaterial(x, y, z)).isAir() || material.isMaterial(DefaultMaterial.LEAVES)
					|| material.isMaterial(DefaultMaterial.LEAVES_2)) && (y > 0))
				y--;
