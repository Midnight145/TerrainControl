...
public class BracketSettingsReader implements SettingsReader {
	...
	public BracketSettingsReader(String name, String rawSettings) {
		...
		for (String setting : settings) {
			...
			else if (settingParts.length == 2) { settingsMap.put(settingName, settingParts[1].trim()); }
		}
	}
	...
	public <T> List<ConfigFunction<T>> getConfigFunctions(T holder, boolean useFallback) {
		...
		if (useFallback && fallback != null) { return fallback.getConfigFunctions(holder, true); }
	...
	@Override
	public Iterable<Entry<String, String>> getRawSettings() { return Collections.emptyList(); }
	...
	public <S> S getSetting(Setting<S> setting, S defaultValue) {
		...
		if (stringValue != null) {
			try {
				...
				TerrainControl.log(LogMarker.ERROR, "The value \"{}\" is not valid for the setting {} in {}: {}", stringValue, setting,
						name, e.getMessage());
			}
		}
	...
	public boolean hasSetting(Setting<?> setting) {
		if (settingsMap.containsKey(setting.getName().toLowerCase())) { return true; }
		if (fallback != null) { return fallback.hasSetting(setting); }
		return false;
	}

	@Override
	public boolean isNewConfig() { return true; }
