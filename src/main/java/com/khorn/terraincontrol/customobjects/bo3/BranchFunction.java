...
public class BranchFunction extends BO3Function implements Branch {
	 ...
	 */
	protected String getConfigName() { return "Branch"; }
	...
	protected double readArgs(List<String> args, boolean accumulateChances) throws InvalidConfigException {
		...
		for (i = 3; i < args.size() - 2; i += 3) {
			...
			if (object == null) {
				throw new InvalidConfigException("The " + this.getConfigName() + " `" + args.get(i)
						+ "` was not found. Make sure to place it in the same directory.");
			}
			else {
			...
			if (accumulateChances) {
				...
				// CustomObjects are inserted into the Set in ascending chance order with Chance
				// being cumulative.
