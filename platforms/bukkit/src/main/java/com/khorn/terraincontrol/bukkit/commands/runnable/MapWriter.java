...
public class MapWriter implements Runnable {
	...
	private int[] getColors(World world) {
		...
		if (bukkitWorld == null) {
			TerrainControl.log(LogMarker.WARN, "BukkitWorld is null :: Make sure you add `{}` to bukkit.yml",
					(Object) world.getWorld().getName());
						...
						try {
							...
							TerrainControl.log(LogMarker.TRACE, "BiomeBuff Idx::{}<{}x/{}z>, Len::{}, ID::{} | Colors Len::{}",
									new Object[] { arrayPosition, x1, z1, biomeBuffer.length,
											WorldHelper.getGenerationId(biomeBuffer[arrayPosition]), colors.length });
						}
					}
				}
			}
