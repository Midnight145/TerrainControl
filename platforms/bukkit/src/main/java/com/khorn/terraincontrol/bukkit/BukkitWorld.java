...
public class BukkitWorld implements LocalWorld {
	...
	public BukkitBiome getBiomeById(int id) throws BiomeNotFoundException {
		...
		if (biome == null) { throw new BiomeNotFoundException(id, Arrays.asList(settings.biomes)); }
	...
	public LocalBiome getBiomeByName(String name) throws BiomeNotFoundException {
		...
		if (biome == null) { throw new BiomeNotFoundException(name, biomeNames.keySet()); }
		return biome;
	}
	...
	public void placePopulationMobs(LocalBiome biome, Random random, ChunkCoordinate chunkCoord) {
		SpawnerCreature.a(this.world, ((BukkitBiome) biome).getHandle(), chunkCoord.getChunkX() * 16 + 8, chunkCoord.getChunkZ() * 16 + 8,
				16, 16, random);
	}
	...
	private Chunk getChunk(int x, int y, int z) {
		...
		if ((indexX == 0 || indexX == 1) && (indexZ == 0 || indexZ == 1)) {
			...
			if (world.chunkProvider.isChunkLoaded(chunkX, chunkZ)) { return world.getChunkAt(chunkX, chunkZ); }
			return null;
		}
	}
	...
	public LocalMaterialData getMaterial(int x, int y, int z) {
		Chunk chunk = this.getChunk(x, y, z);
		if (chunk == null) { return BukkitMaterialData.ofMinecraftBlock(Blocks.AIR, 0); }
	...
	public void setBlock(int x, int y, int z, LocalMaterialData material) {
		...
		world.A(x, y, z);
		if (!world.isStatic) { world.notify(x, y, z); }
	}
	...
	@Override
	public int getHeightCap() { return settings.worldConfig.worldHeightCap; }

	@Override
	public int getHeightScale() { return settings.worldConfig.worldHeightScale; }

	public TCChunkGenerator getChunkGenerator() { return this.generator; }
	...
	public void setChunkGenerator(TCChunkGenerator _generator) { this.generator = _generator; }
	...
	public void attachMetadata(int x, int y, int z, NamedBinaryTag tag) {
		...
		if (tileEntity != null) {
			...
			TerrainControl.log(LogMarker.DEBUG, "Skipping tile entity with id {}, cannot be placed at {},{},{} on id {}",
					new Object[] { nmsTag.getString("id"), x, y, z, getMaterial(x, y, z) });
		}
	}
