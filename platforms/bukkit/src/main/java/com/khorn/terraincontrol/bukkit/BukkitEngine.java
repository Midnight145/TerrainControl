...
public class BukkitEngine extends TerrainControlEngine {
	...
	@Override
	public File getTCDataFolder() { return plugin.getDataFolder(); }

	@Override
	public File getGlobalObjectsDirectory() { return new File(this.getTCDataFolder(), PluginStandardValues.BO_DirectoryName); }
	...
	public LocalMaterialData readMaterial(String input) throws InvalidConfigException {
		...
		if (block != null) { return BukkitMaterialData.ofMinecraftBlock(block, 0); }
	...
	private LocalMaterialData getMaterial0(String input) throws NumberFormatException, InvalidConfigException {
		...
		if (splitIndex == -1) { splitIndex = input.lastIndexOf("."); }
		if (splitIndex != -1) {
		...
		if (block != null) { return BukkitMaterialData.ofMinecraftBlock(block, blockData); }
		DefaultMaterial defaultMaterial = DefaultMaterial.getMaterial(blockName);
		if (defaultMaterial != DefaultMaterial.UNKNOWN_BLOCK) { return BukkitMaterialData.ofDefaultMaterial(defaultMaterial, blockData); }
