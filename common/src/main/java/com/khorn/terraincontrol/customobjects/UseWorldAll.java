...
public class UseWorldAll extends UseWorld {
	@Override
	public String getName() { return "UseWorldAll"; }

	@Override
	public boolean spawnAsTree(LocalWorld world, Random random, int x, int z) {
		for (CustomObject object : world.getConfigs().getCustomObjects()) {
			if (object.hasPreferenceToSpawnIn(world.getBiome(x, z))) { if (object.spawnAsTree(world, random, x, z)) { return true; } }
		}
		return false;
	}
	...
	public boolean process(LocalWorld world, Random rand, ChunkCoordinate chunkCoord) {
		...
		for (CustomObject selectedObject : world.getConfigs().getCustomObjects()) {
			...
			// Process the object
			if (selectedObject.process(world, rand, chunkCoord)) { spawnedAtLeastOneObject = true; }
		}
