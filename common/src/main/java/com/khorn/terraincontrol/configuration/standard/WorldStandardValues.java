...
public class WorldStandardValues extends Settings {
	...
	public static final Setting<String> BIOME_MODE = stringSetting("BiomeMode", "Normal"),
			IMAGE_FILE = stringSetting("ImageFile", "map.png"), IMAGE_FILL_BIOME = stringSetting("ImageFillBiome", "Ocean");

	public static final Setting<Integer> WORLD_HEIGHT_SCALE_BITS = intSetting("WorldHeightScaleBits", 7, 5, 8),
			WORLD_HEIGHT_CAP_BITS = intSetting("WorldHeightCapBits", 8, 5, 8), GENERATION_DEPTH = intSetting("GenerationDepth", 10, 1, 20),
			BIOME_RARITY_SCALE = intSetting("BiomeRarityScale", 100, 1, Integer.MAX_VALUE),
			LAND_RARITY = intSetting("LandRarity", 97, 1, 100), LAND_SIZE = intSetting("LandSize", 0, 0, 20),
			LAND_FUZZY = intSetting("LandFuzzy", 6, 0, 20), ICE_RARITY = intSetting("IceRarity", 90, 1, 100),
			ICE_SIZE = intSetting("IceSize", 3, 0, 20), RIVER_RARITY = intSetting("RiverRarity", 4, 0, 20),
			RIVER_SIZE = intSetting("RiverSize", 0, 0, 20), WATER_LEVEL_MAX = intSetting("WaterLevelMax", 63, WORLD_DEPTH, WORLD_HEIGHT),
			...
			CAVE_RARITY = intSetting("CaveRarity", 7, 0, 100), CAVE_FREQUENCY = intSetting("CaveFrequency", 40, 0, 200),
			...
			CANYON_MIN_LENGTH = intSetting("CanyonMinLength", 84, 1, 500), CANYON_MAX_LENGTH = intSetting("CanyonMaxLength", 112, 1, 500),
			OBJECT_SPAWN_RATIO = intSetting("ObjectSpawnRatio", 1, 1, 1000), STRONGHOLD_COUNT = intSetting("StrongholdCount", 3, 0, 1000),
			STRONGHOLD_SPREAD = intSetting("StrongholdSpread", 3, 1, 1000), VILLAGE_DISTANCE = intSetting("VillageDistance", 32, 9, 10000),
	...
	public static final Setting<Boolean> RIVERS_ENABLED = booleanSetting("RiversEnabled", true),
			RANDOM_RIVERS = booleanSetting("RandomRivers", false), IMPROVED_RIVERS = booleanSetting("ImprovedRivers", false),
			FROZEN_OCEAN = booleanSetting("FrozenOcean", true), EVEN_CAVE_DISTRIBUTION = booleanSetting("EvenCaveDistrubution", false),
			DISABLE_BEDROCK = booleanSetting("DisableBedrock", false), CEILING_BEDROCK = booleanSetting("CeilingBedrock", false),
			FLAT_BEDROCK = booleanSetting("FlatBedrock", false), REMOVE_SURFACE_STONE = booleanSetting("RemoveSurfaceStone", false),
			...
			STRONGHOLDS_ENABLED = booleanSetting("StrongholdsEnabled", true), VILLAGES_ENABLED = booleanSetting("VillagesEnabled", true),
	...
	public static final Setting<List<String>> NORMAL_BIOMES = stringListSetting("NormalBiomes", "Desert", "Forest", "Extreme Hills",
			"Swampland", "Plains", "Taiga", "Jungle"),
			ICE_BIOMES = stringListSetting("IceBiomes", "Ice Plains"),
			ISLE_BIOMES = stringListSetting("IsleBiomes", "MushroomIsland", "Ice Mountains", "DesertHills", "ForestHills", "TaigaHills",
					"River", "JungleHills"),
			BORDER_BIOMES = stringListSetting("BorderBiomes", "MushroomIslandShore", "Beach", "Extreme Hills Edge"),
			CUSTOM_BIOMES = stringListSetting("CustomBiomes");

	public static final Setting<Double> OLD_BIOME_SIZE = doubleSetting("OldBiomeSize", 1.5, 0.1, 10),
			MIN_MOISTURE = doubleSetting("MinMoisture", 0, 0, 1), MAX_MOISTURE = doubleSetting("MaxMoisture", 0, 0, 1),
			MIN_TEMPERATURE = doubleSetting("MinTemperature", 0, 0, 1), MAX_TEMPERATURE = doubleSetting("MaxTemperature", 0, 0, 1),
