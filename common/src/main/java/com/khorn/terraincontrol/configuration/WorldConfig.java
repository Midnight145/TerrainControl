...
public class WorldConfig extends ConfigFile {
		...
		for (Iterator<Entry<String, Integer>> it = customBiomeGenerationIds.entrySet().iterator(); it.hasNext();) {
			...
			if (DefaultBiome.Contain(biomeName)) {
				TerrainControl.log(LogMarker.WARN,
						"CustomBiomes only accepts custom biomes," + " {} is a vanilla biome. Removing it from the list.", biomeName);
				it.remove();
				continue;
			}
			...
			if (biomeId == -1) { entry.setValue(world.getFreeBiomeId()); }
		}
	}
	...
	protected void correctSettings() {
		...
		imageFillBiome = (DefaultBiome.Contain(imageFillBiome) || customBiomeGenerationIds.keySet().contains(imageFillBiome))
				? imageFillBiome
				: WorldStandardValues.IMAGE_FILL_BIOME.getDefaultValue();
	...
	protected void writeConfigSettings(SettingsWriter writer) throws IOException {
		...
		if (this.populateUsingSavedBiomes) {
			...
			writer.comment("map made with for example WorldPainter, but still want to populate it using " + PluginStandardValues.PLUGIN_NAME
					+ ".");
			writer.comment("Using this together with " + BiomeStandardValues.REPLACE_TO_BIOME_NAME + " is discouraged: it uses the biome");
			writer.comment(
					"specified in " + BiomeStandardValues.REPLACE_TO_BIOME_NAME + " to populate the chunk, instead of the biome itself.");
	...
	public enum ImageMode {
		Repeat, Mirror, ContinueNormal, FillEmpty,
	}
