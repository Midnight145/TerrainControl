 ...
 *
 * <p>
 * Note: this is an internal class that is pending a rename. For backwards
...
public class WorldSettings implements ConfigProvider {
	...
	private void loadCustomObjects() {
		...
		if (!FileHelper.migrateFolder(oldWorldObjectsDir, worldObjectsDir)) {
			TerrainControl.log(LogMarker.WARN,
					"Failed to move old world" + " custom objects from {} to {} in world {}." + " Please move the old objects manually.",
		...
		for (BiomeConfig biomeConfig : biomeConfigs.values()) {
			...
			if (generationId < 0 || generationId >= world.getMaxBiomesCount()) {
				TerrainControl.log(LogMarker.ERROR,
						"The biome id of the {} biome, {}, is too high. It must be between 0 and {}, inclusive.", biomeConfig.getName(),
						generationId, world.getMaxBiomesCount() - 1);
			...
			if (!biomeConfig.replaceToBiomeName.isEmpty()) {
				...
				if (replaceToConfig == null) {
					...
					TerrainControl.log(LogMarker.WARN, "Invalid ReplaceToBiomeName in biome {}: biome {} doesn't exist",
			...
			if (!biome.getIds().isVirtual()) { savedBiomes.add(biome); }
			...
			if (this.worldConfig.IceBiomes.contains(biomeConfig.getName())) { this.worldConfig.iceBiomesRarity += biomeConfig.biomeRarity; }
	...
	private void saveSettings() {
		for (LocalBiome biome : this.biomes) {
			if (biome != null) { biome.getBiomeConfig().outputToFile(); }
		}
	}
	...
	@Override
	public LocalBiome[] getBiomeArray() { return this.biomes; }

	@Override
	public CustomObjectCollection getCustomObjects() { return customObjects; }
