...
public class WeightedMobSpawnGroup {
	...
	public String getInternalName() { return MobNames.toInternalName(this.mob); }

	public String getConfigName() { return this.mob; }
	...
	private static WeightedMobSpawnGroup readSingleGroup(String json) throws InvalidConfigException {
		...
		for (String option : groupParts) {
			...
			if (optionParts.length != 2) { throw new InvalidConfigException("Invalid JSON structure near " + option); }
			...
			if (key.equalsIgnoreCase("\"weight\"")) { weight = StringHelper.readInt(value, 0, 1000); }
			if (key.equalsIgnoreCase("\"min\"")) { min = StringHelper.readInt(value, 0, 1000); }
			if (key.equalsIgnoreCase("\"max\"")) { max = StringHelper.readInt(value, 0, 1000); }
		}
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof WeightedMobSpawnGroup)) { return false; }
		WeightedMobSpawnGroup other = (WeightedMobSpawnGroup) obj;
		if (max != other.max || min != other.min || weight != other.weight) { return false; }
		if (!mob.equals(other.mob)) { return false; }
		return true;
	}
