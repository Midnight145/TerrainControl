...
public abstract class ConfigFunction<T> {
	...
	protected final void assureSize(int size, List<String> args) throws InvalidConfigException {
		if (args.size() < size) { throw new InvalidConfigException("Too few arguments supplied"); }
	}
	...
	public final String getError() throws IllegalStateException {
		if (isValid()) { throw new IllegalStateException("Function is valid, so no error"); }
		return error;
	}
	 ...
	 * @return The holder.
	 */
	protected final T getHolder() { return holder; }
	 ...
	 *
	 * <p>
	 * This method is intended to combat Java's type erasure: it provides
	 ...
	 */
	public final void setHolder(T holder) { this.holder = holder; }
	...
	public final void setValid(boolean valid) {
		if (valid == false) { throw new UnsupportedOperationException("Use the invalidate method"); }
		if (valid == true && !isValid()) { throw new UnsupportedOperationException("Revalidating objects is no longer supported"); }
