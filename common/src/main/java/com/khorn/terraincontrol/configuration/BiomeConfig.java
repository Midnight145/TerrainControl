...
public class BiomeConfig extends ConfigFile {
	...
	public void process() {
		if (!processHasRun) {
			...
			if (this.isNewConfig) { this.resourceSequence.addAll(defaultSettings.createDefaultResources(this)); }
	...
	public void outputToFile() {
		if (!processHasRun) { throw new IllegalStateException("Run process() first!"); }
	...
	public SaplingGen getSaplingGen(SaplingType type) {
		...
		if (gen == null && type.growsTree()) { gen = saplingGrowers.get(SaplingType.All); }
		return gen;
	}
	...
	private void ReadResourceSettings() {
		for (ConfigFunction<BiomeConfig> res : reader.getConfigFunctions(this, this.doResourceInheritance)) {
			...
			if (res != null && res.getHolderType() != null) {
				...
				else if (res instanceof Resource) { this.resourceSequence.add((Resource) res); }
			}
		}
	}
	...
	protected void correctSettings() {
		...
		this.replaceToBiomeName = (DefaultBiome.Contain(this.replaceToBiomeName)
				|| this.worldConfig.customBiomeGenerationIds.keySet().contains(this.replaceToBiomeName)) ? this.replaceToBiomeName : "";

		this.riverBiome = (DefaultBiome.Contain(this.riverBiome)
				|| this.worldConfig.customBiomeGenerationIds.keySet().contains(this.riverBiome)) ? this.riverBiome : "";
	}
	...
	protected void renameOldSettings() {
		...
		Setting<?>[] copyFromWorld = { BiomeStandardValues.MAX_AVERAGE_HEIGHT, BiomeStandardValues.MAX_AVERAGE_DEPTH,
				...
				BiomeStandardValues.VOLATILITY_WEIGHT_2, BiomeStandardValues.DISABLE_BIOME_HEIGHT,
				BiomeStandardValues.CUSTOM_HEIGHT_CONTROL };
		...
		if (customTreeChance == 100) { reader.addConfigFunction(ConfigFunction.create(this, SaplingGen.class, "All", "UseWorld", 100)); }
		if (customTreeChance > 0 && customTreeChance < 100) {
			reader.addConfigFunction(ConfigFunction.create(this, SaplingGen.class, SaplingType.Oak, "UseWorld", customTreeChance,
					TreeType.BigTree, 10, TreeType.Tree, 100));
			reader.addConfigFunction(
			...
			reader.addConfigFunction(ConfigFunction.create(this, SaplingGen.class, SaplingType.SmallJungle, "UseWorld", customTreeChance,
		...
		if (replacedBlocksValue.contains("=")) {
			...
			for (String replacedBlock : values) {
				try {
					...
					if (start != -1 && end != -1) { // Found height settings
