...
public class CavesGen extends TerrainGenBase {
	...
	protected void generateCaveNode(long seed, ChunkBuffer generatingChunkBuffer, double x, double y, double z, float paramFloat1,
		...
		for (; angle < maxAngle; angle++) {
			...
			if ((!isLargeCave) && (localRandom.nextInt(4) == 0)) { continue; }
			...
			// Boundaries check.
			if ((x < real_x - 16.0D - d3 * 2.0D) || (z < real_z - 16.0D - d3 * 2.0D) || (x > real_x + 16.0D + d3 * 2.0D)
					|| (z > real_z + 16.0D + d3 * 2.0D))
				continue;
			...
			if (i1 < 1)
				i1 = 1;
			if (i2 > this.worldSettings.worldHeightCap - 8) { i2 = this.worldSettings.worldHeightCap - 8; }
			if (i3 < 0)
				i3 = 0;
			if (i4 > 16)
				i4 = 16;
	...
	protected boolean isSuitableBlock(LocalMaterialData material, LocalMaterialData materialAbove, LocalBiome biome) {
		...
		if (material.equals(biomeConfig.surfaceBlock)) { return true; }

		// Few hardcoded cases
		if (material.isMaterial(DefaultMaterial.HARD_CLAY)) { return true; }
		if (material.isMaterial(DefaultMaterial.SANDSTONE)) { return true; }
		// TODO: add red sandstone case in Minecraft 1.8
		if (material.isMaterial(DefaultMaterial.SNOW)) { return true; }
	...
	protected void generateChunk(ChunkCoordinate chunkCoord, ChunkBuffer generatingChunkBuffer) {
		...
		for (int j = 0; j < i; j++) {
			...
			if (this.worldSettings.evenCaveDistribution)
				y = this.random.nextInt(this.worldSettings.caveMaxAltitude - this.worldSettings.caveMinAltitude)
						+ this.worldSettings.caveMinAltitude;
			else
				y = this.random.nextInt(this.random.nextInt(this.worldSettings.caveMaxAltitude - this.worldSettings.caveMinAltitude) + 1)
						+ this.worldSettings.caveMinAltitude;
			...
			if ((largeCaveSpawned) || (this.random.nextInt(100) <= this.worldSettings.caveSystemPocketChance - 1)) {
				count += this.random.nextInt(this.worldSettings.caveSystemPocketMaxSize - this.worldSettings.caveSystemPocketMinSize)
						+ this.worldSettings.caveSystemPocketMinSize;
			}
			while (count > 0) {
				count--;
