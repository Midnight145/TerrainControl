...
public class WellGen extends Resource {
	...
	public void spawn(LocalWorld world, Random random, boolean villageInChunk, int x, int z) {
		...
		if (!sourceBlocks.contains(sourceBlock)) { return; }
		int i;
		int j;

		for (i = -2; i <= 2; ++i) {
			for (j = -2; j <= 2; ++j) {
				if (world.isEmpty(x + i, y - 1, z + j) && world.isEmpty(x + i, y - 2, z + j)) { return; }
			}
		}
		...
		for (i = -2; i <= 2; ++i) {
			for (j = -2; j <= 2; ++j) {
				if (i == -2 || i == 2 || j == -2 || j == 2) { world.setBlock(x + i, y + 1, z + j, material); }
			}
		}
	...
	public boolean equals(Object other) {
		...
		return this.maxAltitude == compare.maxAltitude && this.minAltitude == compare.minAltitude && this.slab.equals(compare.slab)
				&& this.sourceBlocks.equals(compare.sourceBlocks) && this.water.equals(compare.water);
	}
