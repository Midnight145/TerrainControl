...
public class IceSpikeGen extends Resource {
	...
	protected void load(List<String> args) throws InvalidConfigException {
		...
		if (this.type == null) { throw new InvalidConfigException("Unknown spike type " + typeString); }
	...
	public String makeString() {
		return "IceSpike(" + material + "," + type + "," + frequency + "," + rarity + "," + minAltitude + "," + maxAltitude
	...
	public void spawnBasement(LocalWorld world, Random random, int x, int z) {
		...
		if (!this.sourceBlocks.contains(world.getMaterial(x, y, z))) { return; }
			...
			for (int actualZ = z - radius; actualZ <= z + radius; actualZ++) {
				...
				if (deltaX * deltaX + deltaZ * deltaZ <= radius * radius) {
					for (int deltaY = y - one; deltaY <= y + one; deltaY++) {
						...
						if (this.sourceBlocks.contains(localBlock)) { world.setBlock(actualX, deltaY, actualZ, this.material); }
					}
				}
			}
		}
	...
	public void spawnSpike(LocalWorld par1World, Random random, int x, int z, boolean hugeSpike) {
		...
		if (!sourceBlocks.contains(par1World.getMaterial(x, y, z))) { return; }

		y += random.nextInt(4);
		int var6 = random.nextInt(4) + 7;
		int var7 = var6 / 4 + random.nextInt(2);

		if (var7 > 1 && hugeSpike) { y += 10 + random.nextInt(30); }
			...
			for (var11 = -var10; var11 <= var10; ++var11) {
				...
				for (int var13 = -var10; var13 <= var10; ++var13) {
					...
					if ((var11 == 0 && var13 == 0 || var12 * var12 + var14 * var14 <= var9 * var9)
							&& (var11 != -var10 && var11 != var10 && var13 != -var10 && var13 != var10 || random.nextFloat() <= 0.75F)) {
		...
		for (int var16 = -var8; var16 <= var8; ++var16) {
			...
			while (var10 <= var8) {
				...
				if (Math.abs(var16) == 1 && Math.abs(var10) == 1) { var17 = random.nextInt(5); }
