...
public class MesaSurfaceGenerator implements SurfaceGenerator {
	...
	public static MesaSurfaceGenerator getFor(String settingValue) {
		if (NAME_NORMAL.equalsIgnoreCase(settingValue)) { return new MesaSurfaceGenerator(false, false); }
		if (NAME_FOREST.equalsIgnoreCase(settingValue)) { return new MesaSurfaceGenerator(false, true); }
		if (NAME_BRYCE.equalsIgnoreCase(settingValue)) { return new MesaSurfaceGenerator(true, false); }
		return null;
	}
	...
	private void initializeSmallByteArray(long worldSeed) {
		...
		for (int i = 0; i < 64; ++i) {
			...
			if (i < 64) { this.blockDataValuesArray[i] = 1; }
		}
		...
		for (int i = 0; i < numberFromThreeToFive; i++) {
			...
			for (int j = 0; increasingNumber + j < 64 && j < 1; j++) {
				...
				if (increasingNumber + j > 1 && random.nextBoolean()) { this.blockDataValuesArray[increasingNumber + j - 1] = 8; }

				if (increasingNumber + j < 63 && random.nextBoolean()) { this.blockDataValuesArray[increasingNumber + j + 1] = 8; }
			}
		}
	}
	...
	public void spawn(LocalWorld world, BiomeConfig biomeConfig, double noise, int x, int z) {
		...
		if (this.blockDataValuesArray == null) { this.initializeSmallByteArray(world.getSeed()); }
		...
		if (highestBlockY <= biomeConfig.riverWaterLevel) { foundWater = true; }
		...
		if (this.isBryceMesa && !foundWater) {
			...
			if (bryceNoiseValue > 0.0D) {
				...
				if (bryceHeight > d5) { bryceHeight = d5; }
		...
		if (this.isBryceMesa && !foundWater) { maxHeight = Math.max((int) bryceHeight, maxHeight); }
		...
		for (int y = maxHeight; y >= minHeight; y--) {
			...
			if (blockAtPosition.isAir()) {
				...
				if (j1 == -1) {
					belowSand = false;
					if (y >= waterLevel - 4 && y <= waterLevel + 1) { currentGroundBlock = whiteStainedClay; }
					...
					if (y >= waterLevel - 1) {
						if (this.isForestMesa && y > 86 + noisePlusRandomFactor * 2) {
							...
							blockData = 16;
							if (y >= 64 && y <= 127) {
								if (!cosNoiseIsLargerThanZero) { blockData = this.getBlockData(x, y, z); }
							}
							else {
								blockData = 1;
							}
	...
	public String toString() {
		if (this.isForestMesa) { return NAME_FOREST; }
		if (this.isBryceMesa) { return NAME_BRYCE; }
		return NAME_NORMAL;
	}
