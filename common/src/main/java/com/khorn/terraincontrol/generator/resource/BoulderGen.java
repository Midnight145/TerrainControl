...
public class BoulderGen extends Resource {
	...
	public void spawn(LocalWorld world, Random random, boolean villageInChunk, int x, int z) {
		...
		if (y < this.minAltitude || y > this.maxAltitude) { return; }

		while (y > 3) {
			LocalMaterialData material = world.getMaterial(x, y - 1, z);
			if (sourceBlocks.contains(material)) { break; }
			y--;
		}
		if (y <= 3) { return; }
			...
			for (int i1 = x - k; i1 <= x + k; i1++) {
				for (int i2 = z - n; i2 <= z + n; i2++) {
					for (int i3 = y - m; i3 <= y + m; i3++) {
						...
						float f3 = i2 - z;
						float f4 = i3 - y;
						if (f2 * f2 + f3 * f3 + f4 * f4 <= f1 * f1) { world.setBlock(i1, i3, i2, this.material); }
					}
				}
			}
