...
public class OldBiomeGenerator extends BiomeGenerator {
	...
	public float[] getRainfall(float[] temp_out, int x, int z, int x_size, int z_size) {
		if ((temp_out == null) || (temp_out.length < x_size * z_size)) { temp_out = new float[x_size * z_size]; }
	...
	public int[] getBiomes(int[] paramArrayOfBiomeBase, int x, int z, int xSize, int zSize) {
		...
		this.oldTemperature1 = this.temperatureGenerator1.a(this.oldTemperature1, x, z, xSize, xSize,
				0.025000000372529D / worldConfig.oldBiomeSize, 0.025000000372529D / worldConfig.oldBiomeSize, 0.25D);
		this.oldWetness = this.wetnessGenerator.a(this.oldWetness, x, z, xSize, xSize, 0.0500000007450581D / worldConfig.oldBiomeSize,
				0.0500000007450581D / worldConfig.oldBiomeSize, 0.3333333333333333D);
		this.oldTemperature2 = this.temperatureGenerator2.a(this.oldTemperature2, x, z, xSize, xSize, 0.25D / worldConfig.oldBiomeSize,
				0.25D / worldConfig.oldBiomeSize, 0.5882352941176471D);
		...
		for (int j = 0; j < xSize; j++) {
			for (int k = 0; k < zSize; k++) {
				...
				if (d4 < worldConfig.minTemperature) { d4 = worldConfig.minTemperature; }
				if (d5 < worldConfig.minMoisture) { d5 = worldConfig.minMoisture; }
				if (d4 > worldConfig.maxTemperature) { d4 = worldConfig.maxTemperature; }
				if (d5 > worldConfig.maxMoisture) { d5 = worldConfig.maxMoisture; }
	...
	private static int generatePositionOnBiomeDiagram(double paramFloat1, double paramFloat2) {
		...
		if (paramFloat1 < 0.1F) { return DefaultBiome.PLAINS.Id; }
		if (paramFloat2 < 0.2F) {
			if (paramFloat1 < 0.5F) { return DefaultBiome.PLAINS.Id; }
			if (paramFloat1 < 0.95F) { return DefaultBiome.PLAINS.Id; }
			return DefaultBiome.DESERT.Id;
		}
		if ((paramFloat2 > 0.5F) && (paramFloat1 < 0.7F)) { return DefaultBiome.SWAMPLAND.Id; }
		if (paramFloat1 < 0.5F) { return DefaultBiome.TAIGA.Id; }
		if (paramFloat1 < 0.97F) {
			if (paramFloat2 < 0.35F) { return DefaultBiome.TAIGA.Id; }
			return DefaultBiome.FOREST.Id;
		}

		if (paramFloat2 < 0.45F) { return DefaultBiome.PLAINS.Id; }

		if (paramFloat2 < 0.9F) { return DefaultBiome.FOREST.Id; }
