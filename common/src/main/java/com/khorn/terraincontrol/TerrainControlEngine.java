...
public abstract class TerrainControlEngine {
	...
	public boolean fireCanCustomObjectSpawnEvent(CustomObject object, LocalWorld world, int x, int y, int z) {
		...
		for (EventHandler handler : cancelableEventHandlers) {
			if (!handler.canCustomObjectSpawn(object, world, x, y, z, !success)) { success = false; }
		}
	...
	public boolean fireResourceProcessEvent(Resource resource, LocalWorld world, Random random, boolean villageInChunk, int chunkX,
		...
		boolean success = true;
		for (EventHandler handler : cancelableEventHandlers) {
			if (!handler.onResourceProcess(resource, world, random, villageInChunk, chunkX, chunkZ, !success)) { success = false; }
		}
		for (EventHandler handler : monitoringEventHandlers) {
	 ...
	 */
	public BiomeModeManager getBiomeModeManager() { return biomeManagers; }
	 ...
	 */
	public CustomObjectManager getCustomObjectManager() { return customObjectManager; }
	...
	public void onStart() {
		...
		pluginConfig = new PluginConfig(
				new FileSettingsReader("PluginConfig", new File(getTCDataFolder(), PluginStandardValues.ConfigFilename)));
